-- Restore per-buyer product detail allocation removed in 053_wallet_ngn_native.sql.
-- Each purchase should deliver exactly p_quantity detail items and remove them from inventory.

CREATE OR REPLACE FUNCTION parse_product_detail_items(p_product_details TEXT)
RETURNS TEXT[] AS $$
DECLARE
  v_text TEXT;
  v_items TEXT[];
BEGIN
  v_text := COALESCE(trim(p_product_details), '');
  IF v_text = '' THEN
    RETURN ARRAY[]::TEXT[];
  END IF;

  IF position('<<<ITEM>>>' IN v_text) > 0 THEN
    SELECT COALESCE(array_agg(trim(item)), ARRAY[]::TEXT[])
    INTO v_items
    FROM unnest(string_to_array(v_text, '<<<ITEM>>>')) AS item
    WHERE trim(item) <> '';
...
COMMENT ON FUNCTION purchase_product_with_wallet(UUID, INT)
  IS 'Wallet purchase in NGN. Allocates one product_details item per quantity and stores it on order_items.delivered_details.';
