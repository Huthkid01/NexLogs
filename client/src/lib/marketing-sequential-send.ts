...
export function emailSendPacingSummary() {
  ...
  const batchPauseSec = EMAIL_SEND_BATCH_PAUSE_MS / 1000;
  return `${betweenSec} seconds between each email, ${EMAIL_SEND_BATCH_SIZE} per batch, then a ${batchPauseSec} second pause`;
}
