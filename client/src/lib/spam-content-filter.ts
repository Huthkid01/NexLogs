...
/**
 * Scrub spam trigger phrases from plain text (subject or message).
 */
function phrasePattern(phrase: string) {
  const escaped = escapeRegExp(phrase);
  // Single words get boundaries so "urgent" does not match inside other tokens.
...
/**
 * Scrub spam phrases from HTML while leaving tags/attributes intact enough for email.
 * Phrases inside visible copy and link text are rewritten; href URLs are left alone
 * by only rewriting matches that are not inside tag attribute quotes when possible.
 */
...
export function findRemainingSpamPhrases(text: string): string[] {
  ...
  for (const { phrase } of SPAM_REPLACEMENTS) {
    // Skip punctuation-only markers after scrub — they are cleaned separately.
