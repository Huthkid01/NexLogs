...
import { AlertTriangle, ArrowRight, DollarSign, FolderKanban, LifeBuoy, Mail, Package, RefreshCw, Settings, ShoppingBag, Trash2, Users, Wallet, Wrench } from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  const statCards = [
    ...
    { label: 'Real orders', value: stats?.totalOrders || 0, icon: ShoppingBag, iconClass: 'bg-orange-500/15 text-orange-300' },
    ...
    try {
      ...
      toast.success(
        maintenanceDraft.enabled
          ? 'Maintenance mode is ON — users will see the notice'
        ...
        <CardContent className="space-y-4 p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="admin-heading text-2xl font-semibold">Maintenance mode</h2>
                <p className={cn('mt-1 text-sm', isDark ? 'text-slate-400' : 'text-slate-600')}>
                  When enabled, public users see a scheduled maintenance popup. Admin pages stay usable.
                </p>
              </div>
            </div>
            <Badge
