...
interface HtmlCampaignPreviewModalProps {
  open: boolean;
  onClose: () => void;
  subject: string;
  htmlBody: string;
...
export function HtmlCampaignPreviewModal({
  open,
  onClose,
  subject,
  htmlBody,
    ...
    <div className={adminModalOverlayClass(isDark)}>
      ...
      >
        <div className="flex items-center justify-between border-b px-5 py-4 dark:border-[#18263b]">
          <div>
            <h2 className="text-lg font-semibold">HTML campaign</h2>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          {tab === 'preview' ? (
            <iframe
              title="HTML campaign body"
              srcDoc={htmlBody}
              className="h-[min(70vh,620px)] w-full rounded-xl border border-slate-200 bg-[#f4f6f8] dark:border-[#22324a]"
              sandbox=""
            />
          ) : (
